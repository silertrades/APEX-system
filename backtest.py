...
#   - Long signals only
#   - Symbol-specific thresholds (BTC = 75)
#   - Regime-specific thresholds (mean_reversion = 75)
#
# HOW TO RUN:
...
def build_candles_at_point(all_data: dict, symbol: str,
                            mtf_idx: int, mtf_df: pd.DataFrame) -> dict:
...
def generate_report(results: list) -> str:
    ...
    lines = [
        ...
        "Filters: Long signals only | BTC threshold 75 | "
...
def run_backtest():
    ...
    for symbol in CRYPTO_SYMBOLS:
        ...
        while i < n_candles - FORWARD_CANDLES:
            ...
            # Build data dict matching what live bot passes to scoring engine
            data = {
                "symbol":         symbol,
                "candles":        candles,
            ...
            # Run through the SAME scoring engine as the live bot
            # All filters (longs only, symbol thresholds, regime thresholds)
